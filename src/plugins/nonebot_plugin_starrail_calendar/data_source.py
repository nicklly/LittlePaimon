...
list_api = 'https://hkrpg-api.mihoyo.com/common/hkrpg_cn/announcement/api/getAnnList?game=hkrpg&game_biz=hkrpg_cn&lang=zh-cn&auth_appid=announcement&authkey_ver=1&bundle_id=hkrpg_cn&channel_id=1&level=65&platform=pc&region=prod_gf_cn&sdk_presentation_style=fullscreen&sdk_screen_transparent=true&sign_type=2&uid=100000000'
detail_api = 'https://hkrpg-api-static.mihoyo.com/common/hkrpg_cn/announcement/api/getAnnContent?game=hkrpg&game_biz=hkrpg_cn&lang=zh-cn&bundle_id=hkrpg_cn&platform=pc&region=prod_gf_cn&level=65&channel_id=1'
...
@cache(ttl=timedelta(hours=1), arg_key='url')
...
async def load_event_cn():
    result = await get(url=list_api)
    detail_result = await get(url=detail_api)
    ...
    if result and 'retcode' in result and result['retcode'] == 0 and detail_result and 'retcode' in detail_result and \
        ...
        for data in result['data']['list'][0]['list']:
        ...
        # 光锥/角色活动跃迁
        for data2 in result['data']['pic_list'][0]['type_list'][1]['list']:
            print(result['data']['pic_list'][0]['type_list'][1]['list'])
